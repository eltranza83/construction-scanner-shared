...
describe('Complete Breadth-First Recursive Drive Crawler & Hierarchy Suite', () => {
  ...
  it('8. Full Chain E2E: Drive API -> BFS Crawl -> get_drive_files Tool -> Semantic Intent Evidence -> Final Jarvis Answer', async () => {
    // 1. Google Drive Mock with 3 levels of nesting
    const mockDriveServer = async (url) => {
      const u = String(url);
      if (u.includes('fld_root')) {
        return Response.json({
          files: [
            { id: 'fld_app_folders', name: 'App Folders', mimeType: 'application/vnd.google-apps.folder' },
            { id: 'f_root_plan', name: 'Site Boundary Survey.pdf', mimeType: 'application/pdf', webViewLink: 'https://drive.google.com/file/d/f_root_plan' }
          ]
        });
      }
      if (u.includes('fld_app_folders')) {
        return Response.json({
          files: [
            { id: 'fld_pur_nested', name: 'Google Doc Purchasing List', mimeType: 'application/vnd.google-apps.folder' }
          ]
        });
      }
      if (u.includes('fld_pur_nested')) {
        return Response.json({
          files: [
            { id: 'f_pur_doc_1', name: 'Lot 3 Google Doc Purchasing List.docx', mimeType: 'application/vnd.google-apps.document', webViewLink: 'https://docs.google.com/document/d/f_pur_doc_1/edit' },
            { id: 'f_pur_doc_2', name: 'Lumber & Framing Materials.pdf', mimeType: 'application/pdf', webViewLink: 'https://drive.google.com/file/d/f_pur_doc_2' }
          ]
        });
      }
      return Response.json({ files: [] });
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = mockDriveServer;

    try {
      // Step A: Crawl Google Drive dynamically
      const dynamicDriveTree = await fetchProjectDriveTree('mock_token', 'fld_root');
      assert.ok(dynamicDriveTree);
      assert.equal(dynamicDriveTree.allFiles.length, 3);

      // Step B: User asks: "what's in the Google Doc purchasing list folder"
      // Tool call generated by Jarvis Planner
      const toolCallResult = await executeClientToolCall('get_drive_files', {
        folderName: 'Google Doc Purchasing List'
      }, {
        driveTree: dynamicDriveTree,
        activeProject: { name: 'Lot 3' }
      });

      assert.strictEqual(toolCallResult.found, true);
      assert.strictEqual(toolCallResult.count, 2);
      assert.strictEqual(toolCallResult.folderPath, 'App Folders / Google Doc Purchasing List');

      // Step C: Semantic Intent Grounded Evidence Synthesis (Pass 2)
      const synthesizedAnswer = synthesizeGroundedEvidence([
        {
          success: true,
          tool: { name: 'get_drive_files', args: { folderName: 'Google Doc Purchasing List' } },
          result: toolCallResult
        }
      ], "what's in the Google Doc purchasing list folder", {
        activeProjectName: 'Lot 3',
        driveTree: dynamicDriveTree
      });

      // Step D: Verify Final Jarvis Response
      assert.ok(synthesizedAnswer, 'Jarvis must return a synthesized response');
      assert.match(synthesizedAnswer, /Lot 3 Google Doc Purchasing List\.docx/);
      assert.match(synthesizedAnswer, /Lumber & Framing Materials\.pdf/);
      assert.match(synthesizedAnswer, /App Folders \/ Google Doc Purchasing List/);
    } finally {
      globalThis.fetch = originalFetch;
    }
  });

  it('9. Full Chain E2E: Truthful answer when querying empty nested subfolder 3 levels deep', async () => {
    // Tool call on empty level 4 folder
    const toolCallResult = await executeClientToolCall('get_drive_files', {
      folderName: 'Empty Level 4 Folder'
    }, {
      driveTree: mockDeepDriveTree,
    ...
    assert.strictEqual(toolCallResult.isFolderEmpty, true);

    const synthesizedAnswer = synthesizeGroundedEvidence([
      {
        success: true,
        tool: { name: 'get_drive_files', args: { folderName: 'Empty Level 4 Folder' } },
        result: toolCallResult
      }
    ], "what is inside the Empty Level 4 Folder?", {
      activeProjectName: 'Lot 3',
      driveTree: mockDeepDriveTree
    });

    assert.ok(synthesizedAnswer);
    assert.match(synthesizedAnswer, /Empty Level 4 Folder/);
    assert.match(synthesizedAnswer, /does not currently contain any files/);
  });
